...
def envoyer_telegram(message):
    ...
    try:
        ...
        if r.status_code == 200:
            print(f"[{datetime.now().strftime('%H:%M:%S')}]  Telegram envoy")
        else:
...
# 
#   CALENDRIER CONOMIQUE (ForexFactory)
# 
def get_annonces_eco():
    """
    Scrape ForexFactory pour dtecter les annonces HIGH IMPACT
    dans les 30 prochaines minutes ou passes depuis moins de 30 min.
    ...
    try:
        ...
        # Cherche les vnements HIGH impact (icne rouge sur ForexFactory)
        # ForexFactory marque les HIGH avec "high" dans la classe CSS
        annonces_high = []

        # Pattern pour trouver les vnements avec leur heure et impact
        # ForexFactory structure : heure | devise | impact | vnement
        pattern_high = re.findall(
            r'class="[^"]*impact[^"]*high[^"]*".*?<td class="[^"]*event[^"]*"[^>]*>(.*?)</td>',
            html, re.DOTALL | re.IGNORECASE
        )

        # Pattern alternatif pour les titres d'vnements HIGH
        if not pattern_high:
            pattern_high = re.findall(
        ...
        if annonces_high:
            nom = annonces_high[0]
            print(f"   ForexFactory HIGH IMPACT : {nom}")
            return True, nom

        print(f"   Aucune annonce HIGH IMPACT dtecte")
        return False, None

    except Exception as e:
        print(f"  Calendrier ForexFactory : {e}")
        # En cas d'erreur on bloque par prcaution si heure sensible
        h = datetime.now().hour
        m = datetime.now().minute
        # NFP = 1er vendredi du mois  14h30 heure franaise
...
def get_news_fxstreet(paire):
    """
    Lit le flux RSS FXStreet pour dtecter le sentiment sur la paire.
    Retourne (sentiment, titre_news) : 'BULLISH', 'BEARISH', ou 'NEUTRE'
    """
    try:
        # Mapping paire  mots cls
        mots_paire = {
        ...
        for item in root.findall(".//item")[:20]:
            ...
            # Vrifie si la news concerne notre paire
...
def score_technique(candles):
    ...
    if ema21 and ema50:
        if ema21 > ema50: sb += 1; rb.append(" EMA21 > EMA50 (tendance haussire)")
        else:             ss += 1; rs.append(" EMA21 < EMA50 (tendance baissire)")
    if rsi7:
        if rsi7 < 40:        sb += 1; rb.append(f" RSI7 survendu ({rsi7})")
        elif 40 <= rsi7 <=55:sb += 1; rb.append(f" RSI7 neutre-haussier ({rsi7})")
        if rsi7 > 60:        ss += 1; rs.append(f" RSI7 surachet ({rsi7})")
        elif 45 <= rsi7 <=60:ss += 1; rs.append(f" RSI7 neutre-baissier ({rsi7})")
    if macd_l and macd_s:
        if macd_l > macd_s: sb += 1; rb.append(" MACD croise  la hausse")
        else:               ss += 1; rs.append(" MACD croise  la baisse")
    if macd_h:
        if macd_h > 0: sb += 1; rb.append(" Histogramme MACD positif")
        else:          ss += 1; rs.append(" Histogramme MACD ngatif")
    if vwap:
...
def score_orderflow(candles):
    ...
    else:       ss += 1; rs.append(" CVD ngatif  pression vendeuse")
    ...
    if wick > body * 2 and c0["volume"] > avg_vol * 1.5:
        if c0["close"] > c0["open"]: sb += 1; rb.append(" Absorption haussire")
        else:                        ss += 1; rs.append(" Absorption baissire")

    # Imbalances (FVG)
    for i in range(1, len(candles) - 1):
        prev, nxt = candles[i + 1], candles[i - 1]
        if nxt["low"] > prev["high"]:
            sb += 1; rb.append(f" Imbalance haussire"); break
        if nxt["high"] < prev["low"]:
            ss += 1; rs.append(f" Imbalance baissire"); break
...
def analyser(paire, candles, news_sentiment, news_titre, annonce_eco, nom_annonce):
    ...
    if pertes_consecutives >= 2:
        print(f"   2 pertes conscutives  pause"); return None
    ...
    #  Filtre annonce co 
    if annonce_eco:
        print(f"   Annonce co imminente ({nom_annonce})  signal ignor")
        envoyer_telegram(
            f" <b>ANNONCE CO IMMINENTE</b>\n"
            f"{nom_annonce}\n"
            f"Aucun signal envoy par prcaution."
        )
        return None
    ...
    tendance_txt = {"BULLISH": " Haussire", "BEARISH": " Baissire", "NEUTRE": " Neutre"}[tendance_htf]
    ...
    message = (
        ...
        f"<b>Qualit :</b> {qualite} ({score_total}/13)\n"
...
def reset_si_nouveau_jour():
    ...
    if datetime.now().day != dernier_jour:
        ...
        pertes_consecutives = 0
        dernier_jour        = datetime.now().day
        print(" Nouveau jour  compteurs remis  zro")
...
def main():
    ...
    while True:
        ...
        # Rcupration news et calendrier co (une fois par cycle)
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}]  Lecture FXStreet & Calendrier co...")
